...
//IsVoiceVerifier returns whether the member has a voice verification role
func IsVoiceVerifier(member *discordgo.Member, guild *discordgo.Guild) bool {
	return hasRole(member, Config.VoiceVerifySistersRole) || hasRole(member, Config.VoiceVerifyBrothersRole)
}
